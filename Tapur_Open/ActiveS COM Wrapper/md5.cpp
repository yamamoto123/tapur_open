...
char* MD5::CalcMD5FromFile(const char *s8_Path)
{
...
char* MD5::CalcMD5FromString(const char *s8_Input)
{
    MD5Init();
    MD5Update((unsigned char*)s8_Input, (unsigned long) strlen(s8_Input));
...
char* MD5::CalcMD5FromBuffer(unsigned char *s8_Buffer, unsigned long Size)
{
    MD5Init();
	MD5Update(s8_Buffer, Size);
    return MD5FinalToString();
}
 ...
 * of bytes.
 */
void MD5::MD5Update(unsigned char *buf, unsigned len)
{
    unsigned long t;
...
void MD5::MD5Final(unsigned char digest[16])
{
    ...
    *p++ = 0x80;
    ...
    MD5Transform(ctx.buf, (unsigned long *) ctx.in);
    MD5::byteReverse((unsigned char *) ctx.buf, 4);
 ...
 */
void MD5::MD5Transform(unsigned long buf[4], unsigned long in[16])
{
