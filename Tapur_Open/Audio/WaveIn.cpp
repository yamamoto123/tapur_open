...
//////////////////////////////////////////////////////////////////////
// ノイズフィルタ
//static BOOL waveInFilter(WAVEHDR* pWaveHdr)
//{
//	long i;
//	SHORT shVal = 0;
//	SHORT shMin = 0;
...
//
//	TRY {
//		if (pWaveHdr->dwBufferLength != WAVEIN_BUFFER_SIZE) return TRUE;
//		if (!pWaveHdr->lpData) return TRUE;
//
//		for (i = 0; i < pWaveHdr->dwBytesRecorded / sizeof(short); i++) {
//			shVal = *((SHORT*)(pWaveHdr->lpData) + i);
//			shMin = min(shVal, shMin);
//			shMax = max(shVal, shMax);
//		}
//	
//		if (shMax == 0 && shMin == 0) return TRUE;
//		if ((shMax - shMin) < 3000) {
//			ZeroMemory(pWaveHdr->lpData, pWaveHdr->dwBytesRecorded);
//		}
//	}
//    CATCH_ALL(e)
//    {
...
UINT WaveInWorkerThread( LPVOID pParam )
{
	...
	if( !f.Open(strFile, CFile::modeCreate | CFile::modeWrite | CFile::shareDenyWrite, &e ) ) {
		_stprintf_s(te.errstr, MAX_ERRSTR, _T("File could not be opened:%s, %s"), strFile, e.m_cause);
...
//TRACE(_T("pWaveIn->m_SyncSw=%d\n"), pWaveIn->m_SyncSw);
    ...
    CATCH(CFileException, e)
    {
		...
		default:
			f.Close();
			_stprintf_s(te.errstr, MAX_ERRSTR, _T("File write error:%s, %s"), strFile, e->m_cause);
			te.ErrorMessage(ERRTYPE_LOGONLY, _T(__FILE__), __LINE__, ERR_WAVE, te.errstr);
			goto THREAD_END;
		}
	}
    AND_CATCH_ALL(e)
    {
		...
		_stprintf_s(te.errstr, MAX_ERRSTR, _T("Record error:%s, %s"), strFile, szCause);
		te.ErrorMessage(ERRTYPE_LOGONLY, _T(__FILE__), __LINE__, ERR_WAVE, te.errstr);
    }
	END_CATCH_ALL
    ...
    CATCH_ALL(e)
    {
		_TCHAR   szCause[1024];
 		e->GetErrorMessage(szCause, 1024);
		_stprintf_s(te.errstr, MAX_ERRSTR, _T("File header write error:%s, %s"), strFile, szCause);
		te.ErrorMessage(ERRTYPE_LOGONLY, _T(__FILE__), __LINE__, ERR_WAVE, te.errstr);
		f.Close();
    }
	END_CATCH_ALL
...
#ifdef _DEBUG
_stprintf_s(te.errstr, MAX_ERRSTR, _T("CWinThread:%d"), pThread);
te.ErrorMessage(ERRTYPE_LOGONLY, _T(__FILE__), __LINE__, ERR_WAVE, te.errstr);
#endif
