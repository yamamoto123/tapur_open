...
LPCTSTR szEntryX  = _T("ProgressWinX");
LPCTSTR szEntryY  = _T("ProgressWinY");
...
#endif
    bSuccess = m_wndProgress.Create(dwProgressStyle, TempRect, this, IDC_PRGRS);
    if (!bSuccess) return FALSE;
...
void CProgressWnd::SetText(LPCTSTR fmt, ...)
{
    ...
    va_start(args, fmt);
    _vstprintf_s(buffer, sizeof(buffer)/sizeof(buffer[0]), fmt, args);
    va_end(args);
