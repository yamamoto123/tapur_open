...
public:
	...
	CArray<int, int> aTextBolds;			// Sub item text bolds
};
...
// テキストのBOLD表示（paak）
		if (p->aTextBolds[lplvcd->iSubItem] > 0) {
...
//paak	const int IDX = CListCtrl::InsertItem(nNewPosition, "");
...
void CReportCtrl::PreSubclassWindow() 
{
	...
	SetExtendedStyle(0);
//paak	ASSERT(GetHeaderCtrl() != NULL);
...
// テキストのBOLD表示設定(PAAK)
void CReportCtrl::SetItemTextBold(int nItem, int nSubItem, BOOL bBold, BOOL bRedraw)
{
	const int ROWS = GetItemCount();
	const int COLS = GetColumnCount();
	BOOL bRowValid = nItem >= 0 && nItem < ROWS;
	...
	if (bRowValid && bColValid)
	{
		...
		CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
		ASSERT(p != NULL);
		p->aTextBolds[nSubItem] = (bBold ? 1 : 0);
	}
	else if (bRowValid && !bColValid)
	{
		// apply to whole row for the existing item
