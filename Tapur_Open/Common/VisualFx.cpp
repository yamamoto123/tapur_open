...
// PAAK
#define TAB_TEXT_RATE     0.75	// テキストサイズが正確に取得できないため、補正値

#define TABWND_DEFAULT_ID 0x2578
#define TABWND_HEIGHT     22    // PAAK
...
#define TAB_DEPL          3     // Distance between the tabs and the client area
//#define TAB_DEPL          4     // Distance between the tabs and the client area
#define TAB_MAXLEN        600
...
TTabWnd::TTabWnd()
{
  ...
  m_BrushWhite.CreateSolidBrush(RGB(255,255,255));	//PAAK
...
void TTabWnd::createFont()
{
  ...
  int nLPixY = GetDeviceCaps(wdc.m_hDC, LOGPIXELSY);
	...
	//PAAK 下線付きフォントの定義
	metrics.lfStatusFont.lfUnderline = 1;
	m_FontUline.CreateFontIndirect(&metrics.lfStatusFont);
}
...
int TTabWnd::drawSelTabTop(CDC *pDC, int x, CRect& client, TTabItem *pItem)
{
  ...
  textSize.cx = (long)((double)textSize.cx * TAB_TEXT_RATE);	//paak
  textSize.cx += 4;
...
//PAAK  pDC->SelectObject(&m_PenDGray);
  if (x != 0) {
    pDC->MoveTo(0, y-1);
    pDC->LineTo(x, y-1);
  }
  ...
  pDC->FillSolidRect(x+3,y,textSize.cx+TAB_SPACE-6, TABSEL_HEIGHT-2, 
                    RGB(255,255,255));
//PAAK                    ::GetSysColor(COLOR_BTNFACE));
  ...
  pItem->SetFont(&m_FontUline);
//PAAK  pItem->SetFont(&m_Font);
  pItem->SetRect(rect);
...
//paak 選択されたボトムTABのみ背景を白にする
HBRUSH TTabWnd::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
	...
	if (nCtlColor == CTLCOLOR_STATIC) {
		...
		for (iterator = m_TabList.begin(); iterator != m_TabList.end(); iterator++) {
			...
			ASSERT(pItem != NULL);
			if (pItem->m_bVisible && nIndex == m_nSelectedTab && m_nTabPos == TP_BOTTOM) {
				if (pItem->m_pCaption == pWnd) {
...
//vc6 only	splitter->IsChildPane(this,row,col);
...
BOOL TVisualFramework::Create(CWnd *pWnd)
{
  ...
  for (it = m_ObjectList.begin(); it != m_ObjectList.end(); it++) {
    ...
    if (rc == FALSE) {
//PAAK      TRACE0(_T("Create visual object failed!\n"));
...
// PAAK
BOOL TVisualFramework::SetTabLabel(TVisualObject *pObject, LPCTSTR szLabel)
{
  ASSERT(pObject);
  ASSERT(pObject->m_pWnd);
  ASSERT(pObject->m_pParent);
  ...
  TTabWnd *pTab = (TTabWnd*)pObject->m_pParent;
  int nIndex = pTab->GetTabIndex(pObject->m_pWnd);
  pTab->SetTabLabel(nIndex, szLabel);
  return TRUE;
}
